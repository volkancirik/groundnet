...
class CMN_NOATT(nn.Module):
  def __init__(self, wrd_vocab, pos_vocab, non_vocab, config):
    super(CMN_NOATT, self).__init__()
    self.config= config
    ...
    self.WprojSUB = nn.Linear(self.hdim*4, 300)
    self.WprojOBJ = nn.Linear(self.hdim*4, 300)
    self.WprojREL = nn.Linear(self.hdim*4, 300)
