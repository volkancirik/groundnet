...
class CMN_LOC_DEEP(nn.Module):
  def __init__(self, wrd_vocab, pos_vocab, non_vocab, config):
    super(CMN_LOC_DEEP, self).__init__()
    self.config= config
    ...
    self.Wbox1 = nn.Linear(self.feat_box, self.wdim)
    self.Wbox2 = nn.Linear(self.wdim, self.wdim)
    self.Wbox3 = nn.Linear(self.wdim, self.wdim)

    self.Wout0  = nn.Linear(self.wdim, self.wdim)
    self.Wout1  = nn.Linear(self.wdim, self.wdim)
    self.Wout2  = nn.Linear(self.wdim, 1)
#    self.Wout  = nn.Linear(self.wdim, 1)
